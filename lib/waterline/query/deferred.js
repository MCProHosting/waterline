...
var Q = require('q'),
    ...
    cacheHash = 'waterline-query-cache';
...
/**
 * Add a Cache clause to the criteria object
 *
 * @param {Integer} Timeout in seconds
 * @return this
 */

Deferred.prototype.cache = function(timeout) {
  this._cache = timeout;
...
Deferred.prototype.exec = function(cb) {
  ...
  var runQuery = function (callback) {
    var args = [self._criteria, callback];

    if (self._values) args.splice(1, 0, self._values);
  ...
  // If we were told to cache the query (see the Deferred.cache method), we
  // need to look for it in Redis and try to serve it from there. This is O(1)
  // time -- and it's just Redis -- so it saves a lot of computation power!
  if (self._cache) {

    var redis = require("redis-fantastico").instance;

    // Create the identifier. It's the md5sum of the handler function as a string,
    // paired with the query criteria and parameters. We'll use it for storing
    // the data in Redis
    var identifier = crypto.createHash('md5').update(
        self._context.identity         // Identity of the model we're querying
      + JSON.stringify(self._criteria) // Data criteria
      + JSON.stringify(self._values)   // Bound parameters
    ).digest('hex');
    var cacheKey = cacheHash + ':' + identifier;

    async.waterfall([
      // Search for the identified in redis.
      function (wCb) {
        redis.getSlave().client.GET(cacheKey, wCb);
      },
      // If we found it, callback immediate with the results.
      function (results, callback) {
        if (results) {
          callback('done', JSON.parse(results));
        } else {
          callback(null);
        }
      },
      ...
      function (callback) {
        runQuery(function (err) {
          if (err) {
            ...
            // Force the arguments into an actual array, because we want to
            // apply these to the callback later and apply objects doesn't work!
            callback(null, [].slice.call(arguments));
          }
        });
      },
      // Store the result argument in Redis, then return it down the output function.
      function (results, callback) {
        redis.getMaster().client.SETEX(
          cacheKey,                // Cache key
          self._cache,             // Timeout before the key expires
          JSON.stringify(results), // Stringify the results to store
      ...
      // If we're just "done", don't throw the error.
      if (err && err !== 'done') throw err;
      // Callback with the results.
      cb.apply(this, results);
    });

  } else {
    // If we aren't caching it, just run the query.
    runQuery(cb);
  }
};
...
/**
 * Deep pluck in an object.
 *
 * @param  {string|[]string} search
 * @param  {{}} object
 * @return {*}
 */
function getDeepest(search, object) {
      if (!_.isArray(search)) {
        search = search.split('.');
    }
