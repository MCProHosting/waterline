  ...
  if (self._cache) {
    ...
    async.waterfall([
      ...
      function (results, callback) {
        if (results) {
          callback('done', [null, JSON.parse(results)]);
        } else {
      ...
      function (results, callback) {
        redis.getMaster().client.SETEX(cacheKey, self._cache, JSON.stringify(results[1]), function (err) {
          if (err) throw err;
        });
